...
IsletaDiv <- read.csv("Data/Processed/DiversionSubreachData.csv") %>% 
 ...
 # filter(between(month(dates), 4, 10)) %>% 
  select(dates, Diversion_cfs) %>% 
  mutate(Diversion_cfs = ifelse(Diversion_cfs > 1000, NA, Diversion_cfs)) %>% 
  mutate(Diversion_cfs = na.approx(Diversion_cfs, na.rm = FALSE)) %>% 
  mutate(time = 1:length(dates))

IsletaDiv_ts <- IsletaDiv %>% 
  as_tsibble(index = time)

#decomposition because spate-time not decorrelating
decomposed <- IsletaDiv_ts %>% 
  model(STL(Diversion_cfs ~ trend(), robust = TRUE)) %>% 
  components() 

detrended <- decomposed %>% 
  mutate(detrended = Diversion_cfs - trend)

ggplot(decomposed) +
  geom_line(aes(x = time, y = Diversion_cfs), color = "blue") +
  geom_line(aes(x = time, y = trend), color = "red") +
  geom_line(aes(x = time, y = remainder), color = "green") +
  labs(title = "Original, Detrended, and Trend Components",
       x = "Time",
       y = "Value") +
  theme_minimal()

ts <- detrended$detrended #change based on what gage using
x.obs <- ts
...
par(mfrow=c(1,2))  
out<-stplot(DiversionSignal,m=3,d=d,idt=1,mdt=length(DiversionSignal))
...
fn.out <- false.nearest(DiversionSignal, m.max, d, tw)
...
m <- for_m
Mx <- embedd(DiversionSignal, m=m, d=d)
head(Mx)
...
par(mfrow=c(1,1)) 
embedding(DiversionSignal)
scatterplot3d(Mx, type = "l", main="shadow diversion")
...
write.csv(Mx, "Results/Mx/SanAcaciaDiversion_Mx.csv", row.names = F)
