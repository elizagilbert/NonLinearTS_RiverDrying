...
IsletaSign <- read.csv("Results/IsletaSignal_Irrig.csv") %>% 
  rename(ExtentIsleta = x) 

SanAcaciaSign <- read.csv("Results/SanAcaciaSignal_Irrig.csv") %>% 
  rename(ExtentSanAcacia = x) 


##CCM: Does A cause B? (B xmap A) ####
lib_sequence<-5  #by argument in sequence statement
A<-IsletaSign$ExtentIsleta #forcing process  
B<-SanAcaciaSign$ExtentSanAcacia     #response process 


# theiler window for A ####
mutual.out <- mutual(A, lag.max = 100) #mutual(tseriesChaos) Embedding delay = d 
...
par(mfrow=c(1,2))  
out<-stplot(A,m=3,d=d,idt=1,mdt=length(A))
